...
export default function OnetFullTable({ school }: Props) {
  ...
  const rows = useMemo(() => {
    if (!search) return school.onetp6s;
    return school.onetp6s.filter(m => m.year_exam === search);
  }, [search, school.onetp6s]);

  if (!school.onetp6s?.length) {
  ...
  return (
    <section className="mt-8">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-slate-800">คะแนนสอบ ONET ป.6</h2>
        <select
          value={search}
